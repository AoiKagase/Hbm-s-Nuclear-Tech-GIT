...
public class TileEntityMachineCyclotron extends TileEntityMachineBase
		implements ITickable, IEnergyUser, IFluidHandler, ITankPacketAcceptor {
		...
		this.inventory = new ItemStackHandler(16) {
			...
			public void setStackInSlot(int slot, ItemStack stack) {
				if (stack != null && slot >= 14 && slot <= 15 && stack.getItem() instanceof ItemMachineUpgrade)
					world.playSound(null, pos.getX() + 0.5, pos.getY() + 1.5, pos.getZ() + 0.5,
							HBMSoundHandler.upgradePlug, SoundCategory.BLOCKS, 1.5F, 1.0F);
	...
	private void fillFromContainers(TileEntity tile, int inputSlot, int tagetSlot) {
		...
		if (tile != null && tile instanceof ICapabilityProvider) {
			...
			if (capte.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
					MultiblockHandler.intToEnumFacing(meta).rotateY())) {
				IItemHandler cap = capte.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
						MultiblockHandler.intToEnumFacing(meta).rotateY());
				int[] slots;
				if (tile instanceof TileEntityMachineBase) {
					slots = ((TileEntityMachineBase) tile)
							.getAccessibleSlotsFromSide(MultiblockHandler.intToEnumFacing(meta).rotateY());
	...
	public static boolean isItemATarget(Item item) {
		return item == ModItems.part_lithium || item == ModItems.part_beryllium || item == ModItems.part_carbon
				|| item == ModItems.part_copper || item == ModItems.part_plutonium;
	}

	public boolean tryFillAssemblerCap(IItemHandler container, int[] allowedSlots, TileEntityMachineBase te,
			int inputSlot, int targetSlot) {
		if (allowedSlots.length < 1)
			return false;

		int inputAmount = inventory.getStackInSlot(inputSlot).getMaxStackSize()
				- inventory.getStackInSlot(inputSlot).getCount(); // how many items do we need to fill the stack?
		int targetAmount = inventory.getStackInSlot(targetSlot).getMaxStackSize()
				- inventory.getStackInSlot(targetSlot).getCount(); // how many items do we need to fill the stack?
		...
		for (int slot : allowedSlots) {
			if (container.getStackInSlot(slot) == null || container.getStackInSlot(slot).isEmpty()) { // check next slot
																										// in chest if
																										// it is empty
				continue;
	...
	private int moveItem(IItemHandler container, int inventorySlot, int containerSlot, int amount,
	...
	public void update() {
		if (!world.isRemote) {
				...
				if (canProcess() && power >= defConsumption) {
					...
					if (coolant.getFluidAmount() > 0) {
						...
						if (world.rand.nextInt(chance) == 0)
							ExplosionLarge.spawnTracers(world, pos.getX() + 0.5, pos.getY() + 3.25, pos.getZ() + 0.5,
									1);

						if (countdown > 1000) {
							ExplosionThermo.setEntitiesOnFire(world, pos.getX() + 0.5, pos.getY() + 1.5,
									pos.getZ() + 0.5, 25);
							ExplosionThermo.scorchLight(world, pos.getX(), pos.getY(), pos.getZ(), 7);

							if (countdown % 4 == 0)
								ExplosionLarge.spawnBurst(world, pos.getX() + 0.5, pos.getY() + 3.25, pos.getZ() + 0.5,
										18, 1);
						...
						if (countdown == 1140)
							world.playSound(null, pos.getX() + 0.5, pos.getY() + 1.5, pos.getZ() + 0.5,
									HBMSoundHandler.shutdown, SoundCategory.BLOCKS, 10.0F, 1.0F);
			...
			PacketDispatcher.wrapper.sendToAllAround(new FluidTankPacket(pos, coolant, amat),
					new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 15));
		}
	}
	...
	private void explode() {
		...
		if (rand < 2) {
			world.spawnEntity(EntityNukeExplosionMK5.statFac(world, (int) (BombConfig.fatmanRadius * 1.5),
					pos.getX() + 0.5, pos.getY() + 1.5, pos.getZ() + 0.5).mute());

			NBTTagCompound data = new NBTTagCompound();
			data.setString("type", "muke");
			PacketDispatcher.wrapper.sendToAllAround(
					new AuxParticlePacketNT(data, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5),
					new TargetPoint(world.provider.getDimension(), pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
							250));
			world.playSound(null, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, HBMSoundHandler.mukeExplosion,
					SoundCategory.BLOCKS, 15.0F, 1.0F);
		} else if (rand < 4) {
	...
	public boolean canProcess() {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (inventory.getStackInSlot(i + 6).isEmpty())
				return true;
			if (Library.areItemStacksCompatible(out, inventory.getStackInSlot(i + 6), false)
					&& inventory.getStackInSlot(i + 6).getCount() < out.getMaxStackSize())
				return true;
		}
	...
	public void process() {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (inventory.getStackInSlot(i + 6).getItem() == out.getItem()
					&& inventory.getStackInSlot(i + 6).getItemDamage() == out.getItemDamage()
					&& inventory.getStackInSlot(i + 6).getCount() < out.getMaxStackSize()) {
	...
	public AxisAlignedBB getRenderBoundingBox() {
		return new AxisAlignedBB(pos.getX() - 2, pos.getY(), pos.getZ() - 2, pos.getX() + 3, pos.getY() + 4,
				pos.getZ() + 3);
	}
