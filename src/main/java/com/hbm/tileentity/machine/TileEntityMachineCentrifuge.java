...
public class TileEntityMachineCentrifuge extends TileEntityMachineBase implements ITickable, IEnergyUser {
	...
	public boolean canProcess() {
		...
		if ((inventory.getStackInSlot(2).isEmpty() || (itemStack.length > 0 && itemStack[0] != null
				...
				&&
				(inventory.getStackInSlot(3).isEmpty() || itemStack.length < 2
						|| (itemStack.length > 1 && itemStack[1] != null
								&& inventory.getStackInSlot(3).isItemEqual(itemStack[1])
								&& inventory.getStackInSlot(3).getCount() + itemStack[1].getCount() <= itemStack[1]
										.getMaxStackSize()))
				&&
				(inventory.getStackInSlot(4).isEmpty() || itemStack.length < 3
						|| (itemStack.length > 2 && itemStack[2] != null
								&& inventory.getStackInSlot(4).isItemEqual(itemStack[2])
								&& inventory.getStackInSlot(4).getCount() + itemStack[2].getCount() <= itemStack[2]
										.getMaxStackSize()))
				&&
				(inventory.getStackInSlot(5).isEmpty() || itemStack.length < 4
	...
	private void processItem() {
		if (canProcess()) {
			...
			if (inventory.getStackInSlot(0).isEmpty()) {
				// inventory.setStackInSlot(0, new
				// ItemStack(inventory.getStackInSlot(i).getItem().setFull3D()));
	...
	public int getSpeedLvl() {
		...
		Item item;
		for (int i = 6; i <= 7; i++) {
			item = inventory.getStackInSlot(i).getItem();
			if (item == ModItems.upgrade_speed_3)
				level += 10;
			else if (item == ModItems.upgrade_speed_2)
				level += 2;
			else if (item == ModItems.upgrade_speed_1)
				level += 1;
			else if (item == ModItems.upgrade_screm)
				level += 6;
		}
	...
	public void update() {
		...
		if (!world.isRemote) {
			...
			// PacketDispatcher.wrapper.sendToAllAround(new LoopedSoundPacket(pos.getX(),
	...
	private void detectAndSendChanges() {
		...
		PacketDispatcher.wrapper.sendToAllAround(new AuxElectricityPacket(pos.getX(), pos.getY(), pos.getZ(), power),
				new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 10));
		PacketDispatcher.wrapper.sendToAllAround(new AuxGaugePacket(pos.getX(), pos.getY(), pos.getZ(), progress, 0),
				new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 10));
		PacketDispatcher.wrapper.sendToAllAround(
				new AuxGaugePacket(pos.getX(), pos.getY(), pos.getZ(), isProgressing ? 1 : 0, 1),
