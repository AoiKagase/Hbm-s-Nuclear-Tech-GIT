...
public class TileEntityMachinePress extends TileEntityMachineBase implements ITickable, ICapabilityProvider {
	...
	public final static int maxPower = 200;
	...
	public void update() {
		 ...
		 * Portal portal = new Mirror(world, bottomLeft, bottomLeft.addVector(1, 0, 0),
		 * bottomLeft.addVector(0, 1, 0), bottomLeft.addVector(1, 1, 0), null);
			...
			if (!(world.isBlockIndirectlyGettingPowered(pos) > 0)) {
				...
				if (power >= maxPower / 3) {
					...
					if (inventory.getStackInSlot(1) != ItemStack.EMPTY
						...
						ItemStack stack = PressRecipes.getPressResult(inventory.getStackInSlot(2).copy(),
							...
							if (progress >= maxProgress) {
								...
								if (inventory.getStackInSlot(1).getMaxDamage() > 0) {
									inventory.getStackInSlot(1)
											.setItemDamage(inventory.getStackInSlot(1).getItemDamage() + 1);
								...
								this.world.playSound(null, pos, HBMSoundHandler.pressOperate, SoundCategory.BLOCKS,
										1.5F, 1.0F);
							}
