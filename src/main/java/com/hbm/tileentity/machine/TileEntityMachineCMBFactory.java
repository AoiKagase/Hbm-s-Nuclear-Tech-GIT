...
public class TileEntityMachineCMBFactory extends TileEntityMachineBase
	...
	public boolean isItemValidForSlot(int i, ItemStack stack) {
		switch (i) {
			case 0:
				...
				break;
			case 1:
				if (stack.getItem() == ModItems.ingot_magnetized_tungsten
						|| stack.getItem() == ModItems.powder_magnetized_tungsten)
					return true;
				break;
			case 2:
				...
				break;
			case 3:
				if (stack.getItem() == ModItems.ingot_advanced_alloy
						|| stack.getItem() == ModItems.powder_advanced_alloy)
					return true;
				break;
		}
	...
	public boolean canProcess() {
		...
		if (tank.getFluidAmount() >= 1 && power >= 100000 && !inventory.getStackInSlot(1).isEmpty()
				&& !inventory.getStackInSlot(3).isEmpty()
				&& (inventory.getStackInSlot(4).isEmpty() || inventory.getStackInSlot(4).getCount() <= 60)) {
			boolean flag0 = inventory.getStackInSlot(1).getItem() == ModItems.ingot_magnetized_tungsten
					|| inventory.getStackInSlot(1).getItem() == ModItems.powder_magnetized_tungsten;
			boolean flag1 = inventory.getStackInSlot(3).getItem() == ModItems.ingot_advanced_alloy
					|| inventory.getStackInSlot(3).getItem() == ModItems.powder_advanced_alloy;
	...
	public void update() {
		if (!world.isRemote) {
			...
			if (canProcess()) {
				process();
				if (soundCycle == 0)
					this.world.playSound(null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.ENTITY_MINECART_RIDING,
			...
			PacketDispatcher.wrapper.sendToAllAround(new AuxElectricityPacket(pos, power),
					new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 10));
			PacketDispatcher.wrapper.sendToAllAround(new FluidTankPacket(pos, new FluidTank[] { tank }),
					new TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 10));
