...
public class TileEntityMachineFluidTank extends TileEntityMachineBase
		implements ITickable, IFluidHandler, ITankPacketAcceptor, IControllable {
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) {
		...
		if(slot == 2) return FluidUtil.getFluidContained(stack) != null;
		return slot == 4 && FFUtils.isEmtpyFluidTank(stack);
	}
	...
	public void broadcastControlEvt() {
		ControlEventSystem.get(world).broadcastToSubscribed(this,
				ControlEvent.newEvent("tank_set_mode").setVar("mode", new DataValueFloat(mode)));
	}
