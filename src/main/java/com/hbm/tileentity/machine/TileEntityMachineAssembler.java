...
public class TileEntityMachineAssembler extends TileEntityMachineBase implements ITickable, IEnergyUser {
		...
		if (!world.isRemote) {
			...
			if (te != null) {
				if (te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
					...
					IItemHandler cap = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
							MultiblockHandler.intToEnumFacing(meta).rotateY());
					if (cap != null) {
			...
			if (te2 != null) {
				if (te2.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
						MultiblockHandler.intToEnumFacing(meta).rotateY())) {
					IItemHandler cap = te2.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
							MultiblockHandler.intToEnumFacing(meta).rotateY());
					int[] slots;
	...
	private boolean validateTe(TileEntity te) {
		return te != null && te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null) && te
	...
	public boolean tryFillAssemblerCap(IItemHandler container, int[] allowedSlots, TileEntityMachineBase te) {
		if (allowedSlots.length < 1)
			return false;
		if (AssemblerRecipes.getOutputFromTempate(inventory.getStackInSlot(4)) == null
				|| AssemblerRecipes.getRecipeFromTempate(inventory.getStackInSlot(4)).isEmpty()) // No recipe template
																									// found
			return false;
		else {
			...
			for (AStack recipeIngredient : recipeIngredients) {
				...
				if (assemblerItem.isEmpty())
					...
					possibleAmount = assemblerItem.getMaxStackSize() - assemblerItem.getCount(); // how many items do we
																									// need to fill the
																									// stack?
