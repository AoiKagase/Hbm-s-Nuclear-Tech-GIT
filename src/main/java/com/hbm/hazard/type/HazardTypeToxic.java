...
public class HazardTypeToxic extends HazardTypeBase {
	...
	public void addHazardInformation(EntityPlayer player, List<String> list, float level, ItemStack stack, List<HazardModifier> modifiers) {
        level *= stack.getCount();
		if(level > 10000)
			list.add("§a[" + I18nUtil.resolveKey("adjective.extreme") + " " + I18nUtil.resolveKey("trait.toxic") + "] "+(int)level);
		else if(level > 1000)
			list.add("§a[" + I18nUtil.resolveKey("adjective.veryhigh") + " " + I18nUtil.resolveKey("trait.toxic") + "] "+(int)level);
		else if(level > 100)
			list.add("§a[" + I18nUtil.resolveKey("adjective.high") + " " + I18nUtil.resolveKey("trait.toxic") + "] "+(int)level);
		else if(level > 10)
			list.add("§a[" + I18nUtil.resolveKey("adjective.medium") + " " + I18nUtil.resolveKey("trait.toxic") + "] "+(int)level);
		else if(level > 0)
			list.add("§a[" + I18nUtil.resolveKey("adjective.little") + " " + I18nUtil.resolveKey("trait.toxic") + "] "+level);
	}
}